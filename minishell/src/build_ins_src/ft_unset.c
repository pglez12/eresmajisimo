...
/*   Updated: 2024/05/23 09:34:48 by payllana         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	ft_del_env(t_shellinfo *mini, char *var)
{
	...
	lst = mini->env_lst;
	if (!ft_strncmp(mini->env_lst->name, var, ft_strlen(var)))
		mini->env_lst->content = NULL;
	else
	{
		while (lst)
		{
...
int	ft_unset(char **cmd, t_shellinfo *mini, t_cmd *c)
{
	...
	if (!c->next && c->index == 0)
	{
		while (cmd[i])
		{
			...
			{
				valret = 1;
				ft_err_msg("unset", "not a valid identifier", cmd[i]);
			}
			i++;
		}
		return (valret);

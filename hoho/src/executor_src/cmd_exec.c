...
/*   Updated: 2024/05/28 09:49:52 by payllana         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_builtin_infork(t_cmd *c, t_shellinfo *mini, int valret)
{
...
int	ft_exec_builtin(t_cmd *c, t_shellinfo *mini, int flag)
{
	if (!c->cmd || !c->cmd[0])
		return (1);
	if (flag)
		return (ft_builtin_infork(c, mini, -1));
...
void	ft_exec_cmd(t_shellinfo *mini, t_cmd *cmds)
{
	if (ft_exec_builtin(cmds, mini, 1) == -1)
	{
		...
		{
			ft_err_msg("envp", "don't try that", NULL);
			exit (1);
		}
